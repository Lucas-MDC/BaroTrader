# Login Test Plan

<a id="feature-login"></a>
## Login

| Test Type | Link |
| --- | --- |
| Contract | [Contract](contract_tests.md) |
| Unit | [Unit](unit_tests.md) |
| Integration | [Integration](integration_tests.md) |
| End-to-End | [End-to-End](end_to_end_tests.md) |
| Security | [Security](security_tests.md) |
| Performance and Resilience | [Performance and Resilience](performance_and_resilience_tests.md) |
| Accessibility | [Accessibility](accessibility_tests.md) |
