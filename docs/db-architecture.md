...
## Setup flow (CLI)
- Entry: `db/engine/main.js` (scripts `db:setup`, `db:migrate`, `db:seed`, `db:cleanup`).
- Setup (infra only):
  1. `ensureDatabaseUser` (`db/engine/setup/database.js`): creates the runtime login if missing (admin).
  2. `ensureMigratorUser` (`db/engine/setup/database.js`): creates the migrator login if missing (admin).
  3. `ensureDatabase` (`db/engine/setup/database.js`): creates the application DB (admin, owned by migrator).
- Migrations:
...
- Seed/Test: `runAsUser` (`db/engine/seed/runAs.js`) exercises the runtime model as the app user.
- Cleanup: `db/engine/setup/cleanup.js` drops database/users/roles with safety gates.
...
## SQL layout
- `/db/sql/index.js` centralizes paths via `QueryFile` (cache/minify):
...
- Migrations:
  - `db/migrations/*`: node-pg-migrate wrappers (CommonJS).
  - `db/sql/migrations/*`: SQL-first schema and grants.
